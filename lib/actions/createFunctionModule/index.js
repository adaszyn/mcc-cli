...
module.exports = function(moduleName, cwd = process.cwd()) {
    // Android module
    const projectPath = directoryUtil.getMccProjectRootDirectory();
    const root = path.join(projectPath, moduleName.toLowerCase() + CONFIG.moduleSuffix);
    const srcDir = path.join(root, "src/main/java/com/" + packageName.split(".").join("/"));
    createDirectory(root)
      ...
      const handlerRoot = path.join(projectPath, moduleName.toLowerCase() + CONFIG.handlerSuffix);
      const handlerSrcDir = path.join(handlerRoot, "src/main/java/" + handlerPackageName.split(".").join("/"));
      createDirectory(handlerRoot)
        .then(() => createDirectory(handlerSrcDir))
        .then(() => createFileWithContents(path.join(handlerSrcDir, `${moduleName}Handler.java`), handlerTpl, moduleName, handlerPackageName))
        .then(() => createFileWithContents(path.join(handlerRoot, `build.gradle`), handlerGradleTpl, moduleName, handlerPackageName))
        .then(() => createFileWithContents(path.join(handlerRoot, `.gitignore`), handlerGitignoreTpl, moduleName, handlerPackageName))
